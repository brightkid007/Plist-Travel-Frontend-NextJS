...
const BothPlan = ({ plans = [], currentPlan = null, loading = false }) => {
  ...
  const bothPlans = plans.map((plan) => ({
    id: plan.id,
    ...
    commission_percent: parseFloat(plan.commission_percent) || 0,
  }));
  ...
  return (
    <>
      <div className="row y-gap-20 justify-between">
        <div className="text-20 fw-600 col-auto">Combined Plan</div>
      ...
      {loading ? (
        <div className="d-flex justify-center items-center py-40">
          ...
          <p className="text-14 text-light-1 ml-10">Loading Combined plans...</p>
        </div>
      ) : (
        <div className="row y-gap-20">
          {bothPlans.length === 0 ? (
            <div className="col-12 text-center py-40">
              <p className="text-14 text-light-1">No Combined plans available at the moment.</p>
            </div>
          ) : (
            bothPlans.map((item, index) => (
              <div className="col-md-4" key={index}>
                ...
                >
                  ...
                  <div className="text-14 text-green-3 mb-5">
                    {item.commission_percent || 12}% <span className="text-12">commission per booking</span>
                  </div>
